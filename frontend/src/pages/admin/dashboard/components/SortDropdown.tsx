...
function SortDropdown() {
  ...
  const handleSortAZ = () => {
    console.log("Sortowanie: A-Z");
  };

  const handleSortZA = () => {
    console.log("Sortowanie: Z-A");
  };

  const handleResetSort = () => {
    console.log("Resetowanie sortowania do domyślnego");
  };
  ...
  return (
    <div className="relative inline-block text-left">
      ...
      <button
        ...
        className="flex items-center gap-2 bg-white text-main-navy-blue font-medium text-base py-1.5 px-6 rounded-xl shadow-xl hover:bg-zinc-50 cursor-pointer transition-all duration-300 outline-none border-none"
      >
        <span>{selectedSort}</span>
        <ChevronDown 
          className={`h-4 w-4 text-main-navy-blue transition-transform duration-200 ${
            isOpen ? 'rotate-180' : ''
          }`} 
        />
      </button>
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 mt-2 w-44 rounded-xl bg-white shadow-xl border border-gray-100 z-20 overflow-hidden">
            <div className="py-1">
              <button
