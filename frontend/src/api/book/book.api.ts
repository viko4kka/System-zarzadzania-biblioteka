...
export const bookApi = {
  ...
  removeBook: async (data: RemoveBookDto): Promise<string> => {
    const response = await api.patch<RemoveBookResponseDto>(
      `${baseURL}/remove`,
      data,
    );
  ...
  ): Promise<{ books: Book[]; meta: BooksListMetaDto }> => {
  ...
  ): Promise<{ id: string; publisherName: string }> => {
    const response = await api.post<AddPublisherResponseDto>(
      `/author/addAuthor`,
      data,
    );
  ...
  copyRemove: async (data: RemoveCopyDto): Promise<string> => {
    const response = await api.patch<ApiRemoveCopyResponseDto>(
      `/copy/remove`,
      data,
    );
    return response.message;
  ...
  addAuthor: async (
    ...
    const response = await api.post<AddAuthorResponseDto>(
